...
function destinationsOf(base, argv) {
    switch (base) {
        case 'cp':
        case 'mv':
        case 'rsync':
 ...
 */
function collectSegmentDestinations(cmd, vars, dests) {
    ...
    for (const rawSeg of segments) {
        ...
        if (seg.startsWith('(') && seg.endsWith(')')) {
            if (!collectSegmentDestinations(seg.slice(1, -1), vars, dests))
                return false;
            continue;
        }
...
export function bashWriteDestinations(cmd) {
    ...
    const dests = [];
    if (!collectSegmentDestinations(cmd, vars, dests))
        return [];
