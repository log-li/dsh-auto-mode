...
function trustRoots(allowPaths: readonly string[], cwd?: string): string[] {
  const roots: string[] = [];
  if (cwd) roots.push(realpathSafe(cwd));
  ...
  ctx.on('tools/pre-execute' as any, async (exec: any, next: () => Promise<any>) => {
    ...
    try {
      ...
      const perm = exec.arguments && typeof exec.arguments === 'object'
        ? (exec.arguments as Record<string, unknown>).sandbox_permissions
        : undefined;
