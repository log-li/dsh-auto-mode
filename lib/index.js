...
const AUTO_SENTENCE = 'Approval policy: auto. Every tool call that would normally need ' +
...
export async function askHumanForDecision(ctx, req) {
...
async function decideAuto(ctx, config, req, next, cache, breaker, logger) {
...
export function apply(ctx, rawConfig) {
    ...
    ctx.on('approval/request', (req, next) => {
        ...
        return decideAuto(ctx, config, req, next, cache, breaker, logger).then((outcome) => {
            ...
            if (outcome === 'allowed-once' || outcome === 'always') {
                if (breaker.isTripped(sid)) {
                    breaker.resume(sid);
