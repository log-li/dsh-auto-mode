...
export function ruleMatches(rule, toolName, reason) {
    ...
    if (toolPart !== '*' && toolPart.toLowerCase() !== toolName.toLowerCase())
        return false;
    if (patternPart === undefined || patternPart === '')
        return true;
    if (!reason)
        return false;
